...
import val_detr as val  # for end-of-epoch mAP
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    for mname in OUT_LAYER.keys():
        ...
        if m is not None:
            ...
            dfl_flag = mname in ['DetectDFL', 'OBB', 'YoloText', 'OBBText', 'DetectDFL_xn', 'OBB_xn','RTDETRDecoder']
            if dfl_flag and hasattr(m,'cv2'):
    ...
    if dfl_flag and mname!='RTDETRDecoder':
    ...
    compute_loss = RTDETRDetectionLoss(nc=nc, use_vfl=True)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if dfl_flag:
            if mtext is None:
                tags = ('Epoch', 'gpu_mem', "giou", "class", "bbox", 'labels', 'img_size')
            else:
        ...
        with warnings.catch_warnings():
            ...
            # Scheduler
            scheduler.step()
        for i, (imgs, targets, paths, _, ioa, samples) in pbar:  # batch--> imgs[b,C,H,W]  targets[ntb,1(b)+1(cls)+4(xywh) + 2+4n]
            ...
            with autocast(enabled=amp_flag): # enabled=cuda
                ...
                preds = model.predict(imgs, batch=batch) #preds[5][....]
...
def parse_opt(known=False):
    ...
    #detr
    # opt.data = 'data/coco2017.yaml'
    # opt.cfg = 'models/rtdetr/rtdetr-resnet50.yaml'
    # opt.weights = ''
    # opt.hyp = 'data/hyps/hyp.scratch-dfl.yaml'
    # opt.epochs = 100
    # opt.resume = 'runs/train/exp59/weights/last.pt'
